...
public:
    HeavyPart(int mem){
        depth = mem * 1024 * 8 / ((hll_width*hll_bits+32+32+1) * bucket_num);
        width = bucket_num;
        m = hll_width;
    ...
    int insert(uint32_t src, uint32_t dst, uint32_t& hash_pos, uint32_t& swap_src, uint8_t** swap_value)
    {
        ...
        if(temp[width-1].ca_v[pos] < tmplevel)
        {
            ...
            if(guard_val/min_val >= lambda){
                ...
                *swap_value = new uint8_t[m]{};
                memcpy(*swap_value, temp[min_c].ca_v, m * sizeof(uint8_t));
        ...
        for (int i = 0; i < width-1; i++){
            if(Superspreader[buc][i].key == src){
                if(Superspreader[buc][i].flag){
                    *swap_value = new uint8_t[m]{};
                    memcpy(*swap_value, Superspreader[buc][i].ca_v, m * sizeof(uint8_t));
                    flag= true;
                }
